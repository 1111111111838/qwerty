  ...
  return [
    {
      inlineData: {
        mimeType:
          process.env.YEMOT_AUDIO_MIME_TYPE ||
          'audio/wav',
        data: audioBase64
      }
    }
...
המתקשר ביקש במפורש חיפוש באינטרנט.
חפש מידע עדכני ורלוונטי באמצעות Google Search,
ואז ענה בעברית על השאלה על סמך המידע שמצאת.
אל תציג כתובות אינטרנט.`
      }
    ],
    true
  );
...
צור פתיח קצר בעברית שמזכיר בקצרה את הנושא האחרון,
מאפשר להמשיך משם,
ושואל על מה המתקשר רוצה לדבר עכשיו.

אל תמציא פרטים.
בלי נקודות ובלי מרכאות.`
      }
    ]);
...
async function callHandler(call) {
  ...
  if (
    conversationLog.some(
      x => x.phone === callerPhone
    )
  ) {
    openingPrompt =
      await buildOpeningForCaller(
        callerPhone
      );
  }
  ...
  try {
    while (true) {
      ...
      const active =
        activeCalls.get(activeKey);

      if (active) {
        active.status =
          'הקלטה התקבלה — מוריד הקלטה';
      }
      ...
      /*
       * קודם כל משמיעים למתקשר את התשובה.
       * התמלול לדשבורד נעשה לאחר מכן,
       * כדי שלא יעכב את המענה הטלפוני.
       */
            ...
            [
              {
                type: 'text',
                data:
                  'מצטער הייתה תקלה בהקראת התשובה'
              }
            ],
            {
              prependToNextAction: true
      ...
      /*
       * התמלול מיועד לדשבורד בלבד.
       * אם הוא נכשל, לא מפילים את השיחה.
       */

      try {
        if (active) {
          active.status =
            'שומר את השיחה בדשבורד';
        }

        transcript =
          await transcribeForDashboard(
            audioBase64
          );
      } catch (e) {
        logDetailedError(
...
<head>
<meta charset="utf-8">
<title>AI Phone Line</title>
</head>
<body>
<h1>AI Phone Line Dashboard</h1>
<p>המערכת מחוברת וממתינה לשיחות</p>
</body>
</html>
`)
);
...
async function configureYemotStructure() {
  const apiKey =
    process.env.YEMOT_API_KEY?.trim();
